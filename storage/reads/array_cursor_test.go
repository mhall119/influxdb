...
func TestWindowArrayCursor(t *testing.T) {
...
func TestIntegerFirstArrayCursor(t *testing.T) {
	testcases := []aggArrayCursorTest{
		{
			name:  "window",
			every: 2 * time.Minute,
			...
			want: []*cursors.IntegerArray{
				makeIntegerArray(4, mustParseTime("1970-01-01T00:00:00Z"), 2*time.Minute, func(i int64) int64 { return 2 * i }),
			},
		},
	}
	for _, tc := range testcases {
